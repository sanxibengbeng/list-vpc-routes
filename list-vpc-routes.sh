...
# 写入 CSV 头部
echo "VPC_ID,VPC_Name,CIDR_Block,Route_Table_ID,Route_Table_Name,Destination_CIDR,Target,State" > "$OUTPUT_FILE"
...
if [ -z "$VPCS" ] || [ "$VPCS" = "[]" ]; then
  ...
  echo "No VPCs found. CSV output written to $OUTPUT_FILE."
  exit 0
fi
...
echo "$VPCS" | jq -c '.[]' | while read -r VPC; do
  ...
  if [ -z "$ROUTE_TABLES" ] || [ "$ROUTE_TABLES" = "[]" ]; then
    echo "$VPC_ID,$VPC_NAME,$CIDR_BLOCK,,,No Route Tables,," >> "$OUTPUT_FILE"
    continue
  fi
  ...
  echo "$ROUTE_TABLES" | jq -c '.[]' | while read -r RT; do
    ...
    if [ -z "$ROUTES" ] || [ "$ROUTES" = "[]" ]; then
      echo "$VPC_ID,$VPC_NAME,$CIDR_BLOCK,$RT_ID,$RT_NAME,No Routes,," >> "$OUTPUT_FILE"
    else
      # 遍历路由规则并写入 CSV
      echo "$ROUTES" | jq -c '.[]' | while read -r ROUTE; do
        ...
        echo "$VPC_ID,$VPC_NAME,$CIDR_BLOCK,$RT_ID,$RT_NAME,$DEST,$TARGET,$STATE" >> "$OUTPUT_FILE"
      done
    fi
  done
done
